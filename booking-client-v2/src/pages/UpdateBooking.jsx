...
const UpdateBooking = () => {
  ...
  const handleSubmitForm = async () => {
    try {
      const addFind = booking.patientaddr;
      const accounts = await web3.eth.getAccounts();
      const dataField = await record.methods
        .searchAppointment(addFind)
        .call({ from: accounts[0] });
      setBooking({
        ...booking,
        date: dataField[2],
        time: dataField[3],
        diagnosis: dataField[4],
        prescription: dataField[5],
        description: dataField[6],
        status: dataField[7],
      });
